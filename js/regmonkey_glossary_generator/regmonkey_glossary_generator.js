document.addEventListener("DOMContentLoaded", () => {
  ...
  // Used for terms (def), which are plain text.
  const renderInline = (s) => escapeHtml(s)
    .replace(/`([^`]+)`/g, "<code>$1</code>");

  // Descriptions may contain author-authored HTML (e.g. <a> links), so they
  // are treated as trusted markup rather than escaped. Backticks still expand
  // to <code> as a shorthand. Content comes from the author's own YAML, so
  // raw HTML is intentional and not an injection surface.
  const renderDescription = (s) => s
    .replace(/`([^`]+)`/g, "<code>$1</code>");
  ...
  document.querySelectorAll(".glossary-container").forEach(container => {
    ...
    glossary.forEach(item => {
      ...
      dd.innerHTML = renderDescription(item.description).trim();
